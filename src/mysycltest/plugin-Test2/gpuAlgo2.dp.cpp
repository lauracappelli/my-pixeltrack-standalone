...
namespace {
  ...
  SYCL_EXTERNAL void matrixMulVector(const T *a, const T *b, T *c, int numElements, sycl::nd_item<3> item_ct1) {
    int row = item_ct1.get_group(2) * item_ct1.get_local_range().get(2) + item_ct1.get_local_id(2);
...
cms::cuda::device::unique_ptr<float[]> gpuAlgo2(sycl::queue *stream) {
  ...
  */
  ...
  sycl::range<3> threadsPerBlock3{1, NUM_VALUES, NUM_VALUES};
  sycl::range<3> blocksPerGrid3{1, 1, 1};
  if (NUM_VALUES * NUM_VALUES > 32) {
    threadsPerBlock3[2] = 32;
    threadsPerBlock3[1] = 32;
    blocksPerGrid3[2] = ceil(double(NUM_VALUES) / double(threadsPerBlock3[2]));
    blocksPerGrid3[1] = ceil(double(NUM_VALUES) / double(threadsPerBlock3[1]));
  }
  /*
  DPCT1049:41: The workgroup size passed to the SYCL kernel may exceed the limit. To get the device limit, query info::device::max_work_group_size. Adjust the workgroup size if needed.
  */
  stream->submit([&](sycl::handler &cgh) {
    // helper variables defined
    auto d_a_get_ct0 = d_a.get();
    ...
    cgh.parallel_for(sycl::nd_range<3>(blocksPerGrid3 * threadsPerBlock3, threadsPerBlock3),
                     [=](sycl::nd_item<3> item_ct1) {
                       vectorProd(d_a_get_ct0, d_b_get_ct1, d_ma_get_ct2, NUM_VALUES_ct3, item_ct1);
                     });
  });
  /*
  DPCT1049:42: The workgroup size passed to the SYCL kernel may exceed the limit. To get the device limit, query info::device::max_work_group_size. Adjust the workgroup size if needed.
  */
  stream->submit([&](sycl::handler &cgh) {
    // helper variables defined
    auto d_a_get_ct0 = d_a.get();
  ...
  /*
  DPCT1049:43: The workgroup size passed to the SYCL kernel may exceed the limit. To get the device limit, query info::device::max_work_group_size. Adjust the workgroup size if needed.
  */
  stream->submit([&](sycl::handler &cgh) {
    // helper variables defined
  ...
  /*
  DPCT1049:44: The workgroup size passed to the SYCL kernel may exceed the limit. To get the device limit, query info::device::max_work_group_size. Adjust the workgroup size if needed.
  */
  stream->submit([&](sycl::handler &cgh) {
    // helper variables defined
