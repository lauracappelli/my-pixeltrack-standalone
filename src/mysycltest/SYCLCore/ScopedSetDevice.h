  ...
  namespace cuda {
    ...
    public:
      explicit ScopedSetDevice(int newDevice) {
        cudaCheck(prevDevice_ = dev_mgr::instance().current_device_id());
        /*
        DPCT1003:4: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
        */
        cudaCheck((dev_mgr::instance().select_device(newDevice), 0));
      }
      ...
      ~ScopedSetDevice() {
        ...
        // doomed anyway.
        dev_mgr::instance().select_device(prevDevice_);
      }
