...
namespace cms {
  namespace sycltools {
    ...
    typename impl::make_unique_selector<T>::non_array make_unique(sycl::queue stream) {
      ...
      return typename impl::make_unique_selector<T>::non_array{reinterpret_cast<T *>(mem),
                                                                              impl::Deleter{stream}};
    }

    template <typename T>
    typename impl::make_unique_selector<T>::unbounded_array make_unique(size_t n,
                                                                                              sycl::queue stream) {
      ...
      return typename impl::make_unique_selector<T>::unbounded_array{
          reinterpret_cast<element_type *>(mem), impl::Deleter{stream}};
    }
    ...
    template <typename T>
    typename impl::make_unique_selector<T>::non_array make_unique_uninitialized(
        sycl::queue stream) {
      CachingAllocator& allocator = getCachingAllocator(stream.get_device()); 
      void* mem = allocator.allocate(sizeof(element_type), stream);
      return typename impl::make_unique_selector<T>::non_array{reinterpret_cast<T *>(mem),
                                                                              impl::Deleter{stream}};
    }

    template <typename T>
    typename impl::make_unique_selector<T>::unbounded_array make_unique_uninitialized(
    ...
    template <typename T, typename... Args>
    typename impl::make_unique_selector<T>::bounded_array make_unique_uninitialized(Args &&...) =
        delete;
  }  // namespace sycltools
}  // namespace cms
